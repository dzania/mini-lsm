...
#[test]
fn test_task2_merge_tombstone_precedence() {
    fn b(s: &str) -> Bytes {
        Bytes::from(s.to_string())
    }

    let iter1 = MockIterator::new(vec![
        (b("b"), Bytes::new()), // delete marker
        (b("c"), b("4")),
        (b("d"), b("5")),
    ]);

    let iter2 = MockIterator::new(vec![(b("a"), b("1")), (b("b"), b("2")), (b("c"), b("3"))]);

    let iter3 = MockIterator::new(vec![(b("e"), b("4"))]);

    let mut iter = MergeIterator::create(vec![Box::new(iter1), Box::new(iter2), Box::new(iter3)]);
